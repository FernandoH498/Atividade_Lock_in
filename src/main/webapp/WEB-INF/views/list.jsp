						...
						<nav class="header-nav">
							...
							<c:choose>
								<c:when test="${not empty sessionScope.userId}">
									<span class="header-user">👤 ${sessionScope.userNome}</span>
									<a href="${pageContext.request.contextPath}/logout"
										class="btn btn-sm btn-outline">Sair</a>
								</c:when>
								<c:otherwise>
									<a href="${pageContext.request.contextPath}/login"
										class="btn btn-sm btn-outline">Entrar</a>
								</c:otherwise>
							</c:choose>
						</nav>
					</div>
				...
				<section class="hero">
					<div class="hero-inner">
						...
						<div class="hero-actions">
							...
							<a href="${pageContext.request.contextPath}/itens/novo" class="btn-hero-secondary">📦 Achei
								algo</a>
						</div>
					</div>
				</section>
						...
						<form method="get" action="${pageContext.request.contextPath}/itens" class="filter-bar">
							<div class="filter-group">
								...
								<select name="status" id="status">
									...
									<option value="ACHADO" <c:if test="${filtroStatus == 'ACHADO'}">selected</c:if>
										>Achado</option>
									<option value="DEVOLVIDO" <c:if test="${filtroStatus == 'DEVOLVIDO'}">selected
										</c:if>>Devolvido</option>
								</select>
							</div>
							<div class="filter-group">
								...
								<select name="categoria" id="categoria">
									...
									<option value="ELETRÔNICOS" <c:if test="${filtroCategoria == 'ELETRÔNICOS'}">
										selected</c:if>>Eletrônicos</option>
									<option value="DOCUMENTOS" <c:if test="${filtroCategoria == 'DOCUMENTOS'}">selected
										</c:if>>Documentos</option>
									<option value="ROUPAS" <c:if test="${filtroCategoria == 'ROUPAS'}">selected</c:if>
										>Roupas</option>
									<option value="ACESSÓRIOS" <c:if test="${filtroCategoria == 'ACESSÓRIOS'}">selected
										</c:if>>Acessórios</option>
									<option value="OUTROS" <c:if test="${filtroCategoria == 'OUTROS'}">selected</c:if>
										>Outros</option>
								</select>
							</div>
							<div class="filter-group">
								<label for="busca">Buscar descrição</label>
								<input type="text" name="busca" id="busca" placeholder="Ex: mochila, carteira..."
									...
									<article class="item-card">
										...
										<div class="item-card-photo">
											<c:choose>
												<c:when test="${not empty item.fotoPath}">
													<img src="${pageContext.request.contextPath}/uploads/<c:out value="
														${item.fotoPath}" />"
													alt="Foto do item">
												</c:when>
												<c:otherwise>
											...
											<c:choose>
												<c:when test="${item.status == 'ACHADO'}">
													<span class="badge badge-achado badge-overlay">&#128269;
														Achado</span>
												</c:when>
												<c:otherwise>
													<span class="badge badge-devolvido badge-overlay">&#10003;
														Devolvido</span>
												</c:otherwise>
											</c:choose>
										</div>
										...
										<div class="item-card-body">
											...
											<div class="item-card-meta">
												<span>&#128205;
													<c:out value="${item.localEncontrado}" />
												</span>
